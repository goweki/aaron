"use client";

import { useState, useTransition } from "react";
import { Plus, Trash2, SignalHigh, Loader2 } from "lucide-react";
import toast from "react-hot-toast";
import {
  createBroadcasterAction,
  deleteBroadcasterAction,
} from "@/actions/dashboard-actions/broadcaster-actions";
import { BroadcasterWithRelations } from "@/actions/dashboard-actions/dashboard-types";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";

export default function BroadcastersView({
  broadcasters,
}: {
  broadcasters: BroadcasterWithRelations[];
}) {
  const [name, setName] = useState("");
  const [streamUrl, setStreamUrl] = useState("");
  const [description, setDescription] = useState("");
  const [isPending, startTransition] = useTransition();

  const createBroadcaster = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!name.trim()) {
      toast.error("Name is required.");
      return;
    }

    startTransition(async () => {
      const result = await createBroadcasterAction({
        name: name.trim(),
        description: description.trim() || undefined,
        streamUrl: streamUrl.trim() || undefined,
      });

      if (!result.ok) {
        toast.error(result.error);
        return;
      }

      toast.success("Broadcaster created.");
      setName("");
      setStreamUrl("");
      setDescription("");
    });
  };

  const handleDelete = (id: string) => {
    if (!confirm("Delete this broadcaster?")) return;
    startTransition(async () => {
      const result = await deleteBroadcasterAction(id);
      if (!result.ok) {
        toast.error(result.error);
        return;
      }
      toast.success("Broadcaster removed.");
    });
  };

  return (
    <div className="space-y-8">
      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6 dark:border-slate-800 dark:bg-slate-950">
        <h2 className="text-lg font-semibold">Register Broadcaster</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
          Add a broadcast channel and stream metadata for monitoring session
          capture.
        </p>
        <form
          onSubmit={createBroadcaster}
          className="grid gap-4 md:grid-cols-3"
        >
          <div className="space-y-2">
            <Label htmlFor="broadcaster-name">Name</Label>
            <Input
              id="broadcaster-name"
              value={name}
              onChange={(event) => setName(event.target.value)}
              required
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="broadcaster-stream">Stream URL</Label>
            <Input
              id="broadcaster-stream"
              value={streamUrl}
              onChange={(event) => setStreamUrl(event.target.value)}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="broadcaster-description">Description</Label>
            <Input
              id="broadcaster-description"
              value={description}
              onChange={(event) => setDescription(event.target.value)}
            />
          </div>
          <div className="md:col-span-3 text-right">
            <Button
              type="submit"
              disabled={isPending}
              className="inline-flex items-center gap-2"
            >
              {isPending ? <Loader2 className="animate-spin" /> : <Plus />}
              Create Broadcaster
            </Button>
          </div>
        </form>
      </div>

      <div className="rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead className="bg-slate-50 dark:bg-slate-900 text-slate-600 dark:text-slate-400 font-medium">
              <tr>
                <th className="px-4 py-3">Broadcaster</th>
                <th className="px-4 py-3">Stream URL</th>
                <th className="px-4 py-3">Sessions</th>
                <th className="px-4 py-3">Detections</th>
                <th className="px-4 py-3">Status</th>
                <th className="px-4 py-3 text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {broadcasters.map((broadcaster) => (
                <tr
                  key={broadcaster.id}
                  className="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors"
                >
                  <td className="px-4 py-3 font-medium text-slate-900 dark:text-slate-100">
                    <div className="flex items-center gap-2">
                      <SignalHigh className="w-4 h-4 text-indigo-500" />
                      {broadcaster.name}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-xs text-slate-500 dark:text-slate-400 break-all">
                    {broadcaster.streamUrl || "—"}
                  </td>
                  <td className="px-4 py-3">
                    {broadcaster.monitoringSessions.length}
                  </td>
                  <td className="px-4 py-3">{broadcaster.detections.length}</td>
                  <td className="px-4 py-3">{broadcaster.status}</td>
                  <td className="px-4 py-3 text-right">
                    <button
                      type="button"
                      onClick={() => handleDelete(broadcaster.id)}
                      disabled={isPending}
                      className="inline-flex items-center justify-center rounded-md border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-medium text-red-600 transition hover:bg-red-50 disabled:opacity-50"
                    >
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      </div>
    </div>
  );
}
