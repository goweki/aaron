...
export default function UserHomePage() {
  ...
  return authStatus === "loading" || !userSession?.user ? (
    <LoaderHourglass isLoading={authStatus === "loading"} />
  ) : (
    <main className="grid flex-1 items-start gap-4 p-2 sm:p-6 sm:pt-2 md:gap-8 lg:grid-cols-3 xl:grid-cols-3">
      <div className="grid auto-rows-max items-start gap-4 md:gap-8 lg:col-span-2">
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4">
          <Card className="sm:col-span-2" x-chunk="dashboard-05-chunk-0">
            <CardHeader className="pb-3">
              <CardTitle>
                {wish() + titleCase(userSession.user.name?.split(" ")[0] || "")}
              </CardTitle>
              <CardDescription className="max-w-lg text-balance leading-relaxed">
                Aaron is here to help you analyse audio streams and broadcasts.
              </CardDescription>
            </CardHeader>
            <CardFooter>
              <Button>Add new stream</Button>
            </CardFooter>
          </Card>
